...
It currently offers support for the PC version of:
...
The **Unlinker** tool allows loading any fastfile (.ff) and works standalone.
While being able to load any asset it can currently only dump [a subset of these assets](docs/SupportedAssetTypes.md) to disk.
The formats of the dumped assets are either as close as possible to the originally used formats or a commonly used format.

The **Linker** tool allows building custom fastfiles (.ff).
It can also load existing zones and reuse their assets from in-memory.
Only [a subset of asset types](docs/SupportedAssetTypes.md) can be read from disk.

The **ImageConverter** tool allows converting images.
It can convert any .dds to .iwi and vice versa.
...
When this is done you will have a `build` folder with a `OpenAssetTools.slnx` solution file within that you can open with Visual Studio.
